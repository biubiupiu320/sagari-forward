    ...
    <div class="sign-up">
        <div class="title">
            <el-alert :closable="false">
                <template v-slot:title>
                    该QQ账号尚未绑定，请绑定或在此注册本站账号，绑定或注册成功后，可以使用QQ账号直接登录本站
                </template>
            </el-alert>
        </div>
        ...
        <div v-show="type">
            <el-input type="text"
                      ...
                      v-model="already"></el-input>
            <div class="bind-qq-btn">
                <el-button type="primary"
                           size="medium"
                           @click="bindAccount">绑定</el-button>
            </div>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        name: "QQSignUp",
        created() {
            this.formData.avatar = this.$route.params.avatar;
            this.formData.username = this.$route.params.username;
            this.formData.gender = this.$route.params.gender;
            this.formData.qqId = this.$route.params.openId;
